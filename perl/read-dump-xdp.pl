...
sub printHeader
{
    ...
    my $deliveryFlag = substr($bytes, 2, 1);
    my $count     = substr($bytes, 3, 1);
    my $seq       = substr($bytes, 4, 4);
    my $sendTime  = substr($bytes, 8, 4);
    my $sendTimeNS = substr($bytes, 12, 4);
    my $msgCount  = substr($bytes, 16, 1);
    my $dropcount = ChangeByteOrder (chr(0) . substr($bytes, 17, 3));
    my $expected  = ChangeByteOrder substr($bytes, 20, 4);
    
    print "\nBATS Packet Header\n";
    print "-----------------\n";
    printf("Length:       %d\n", unpack "S", $length);
    printf("DeliveryFlag: %d\n", ord($deliveryFlag));
    printf("Count:        %d\n", ord($count));
    printf("Sequence:     %d\n", unpack "L", $seq);
    printf("SendTime:     %d\n", unpack "L", $sendTime);
    printf("SendTimeNS:   %d\n", unpack "L", $sendTimeNS);
