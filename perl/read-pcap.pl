...
sub printHeader
{
    ...
    my $msgsize = ChangeByteOrder substr($bytes, 0, 2);
       
    my $msgtype = ChangeByteOrder substr($bytes, 2, 2);
    
    my $seq     = ChangeByteOrder substr($bytes, 4, 4);
    
    my $sendtime = ChangeByteOrder substr($bytes, 8, 4);
    
    my $productID = substr($bytes, 12, 1);
    
    my $retransflag = substr($bytes, 13, 1);
    
    my $numBodies = substr($bytes, 14, 1);
    
    my $filler = substr($bytes, 15, 1);
